...
import org.phpaspect.weaver.ast.nodes.Statement;
import org.phpaspect.weaver.ast.nodes.StructuralPropertyDescriptor;
...
public final class ListRewrite {
	...
	private ASTNode createTargetNode(ASTNode first, ASTNode last, boolean isMove, ASTNode replacingNode, TextEditGroup editGroup) {
		...
		ASTNode placeholder= nodeStore.newPlaceholderNode(first.getType()); // revisit: could use list type
		if (placeholder == null) {
	...
	private boolean isInsertBoundToPreviousByDefault(ASTNode node) {
		return (node instanceof Statement || node instanceof FieldsDeclaration);
	}
